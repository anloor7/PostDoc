  ...
  for (i2 in grid_f_p) {
    ...
    for (j in 1 : trials) {
      ...
      for (i in 1 : n_series) {
        ...
        cluster_1[[i]] <- transformation(simulate_qar_2(0.2, 1.2, 0.5, i1), simulate_qar_2(0.2, 1.2, 0.4, i1))
        cluster_2[[i]] <- transformation(simulate_qar_2(-0.2, -1.2, 0.5, i1), simulate_qar_2(0.2, 1.2, 0.4, i1))
...
legend_size <- 10
grid_f_p <- seq(1.1, 4, 0.1)
load('vector_q_2_1.RData')
load('vector_fl_2_1.RData')
load('vector_j_2_1.RData')
load('vector_noncircular_2_1.RData')
...
plot_3 = ggplot(df) + geom_line(aes(x = vector_m_repeated, 
  ...
  ggtitle(TeX('Scenario 5 ($\\eta_1$)')) + ylim(c(0, 1))



plot_3_auxiliary = ggplot(df) + geom_line(aes(x = vector_m_repeated, 
                                              y = vector_results, 
                                              colour = vector_color), size = 0.7) +
  scale_colour_manual(values = c('red', 'orange', 'blue', 'green'), 
  ...
  xlab('Fuzziness parameter (m)') + ylab('Success rate') +
  ggtitle(TeX('Scenario 5')) + ylim(c(0, 1))
...
load('vector_q_2_2.RData')
load('vector_fl_2_2.RData')
load('vector_j_2_2.RData')
load('vector_noncircular_2_2.RData')
...
plot_4 = ggplot(df) + geom_line(aes(x = vector_m_repeated, 
  ...
  theme(axis.text = element_text(size = axis_size),
        ...
        plot.title = element_text(hjust = 0.5, size = title_size)) +
  xlab('Fuzziness parameter (m)') + ylab('Success rate') +
  ggtitle(TeX('Scenario 5 ($\\eta_2$)')) + ylim(c(0, 1))
