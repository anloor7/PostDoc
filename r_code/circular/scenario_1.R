...
for (i in 1 : n_series) {
  ...
  cluster_2[[i]] <- transformation(arima.sim(n = i1, list(ar = c(-0.2, 0.2, -0.2), ma = -c(0, 0, 0)),
                                       sd = 1), arima.sim(n = i1, list(ar = c(0.1, 0.1, 0.1), ma = c(0.1, 0.1, 0.1)),
                                                          sd = 1))
  cluster_3[[i]] <- transformation(arima.sim(n = i1, list(ar = c(0, 0, 0), ma = c(0.2, -0.2, 0.2)),
                                       sd = 1), arima.sim(n = i1, list(ar = -c(0.15, 0.15, 0.15), ma = -c(0.15, 0.15, 0.15)),
                                                          sd = 1))
...
set.seed(4)
      ...
      for (i in 1 : 50) {
        ...
        cluster_1[[i]] <- transformation(arima.sim(n = 2000, list(ar = c(0.2, -0.2, 0.2), ma = c(0, 0, 0)),
                                                   sd = 1), arima.sim(n = i1, list(ar = c(0.2, 0.2, 0.2), ma = c(0.2, 0.2, 0.2)),
                                                                      sd = 1))
        cluster_2[[i]] <- transformation(arima.sim(n = 2000, list(ar = c(-0.2, 0.2, -0.2), ma = -c(0, 0, 0)),
                                                   sd = 1), arima.sim(n = i1, list(ar = c(0.1, 0.1, 0.1), ma = c(0.1, 0.1, 0.1)),
                                                                      sd = 1))
        cluster_3[[i]] <- transformation(arima.sim(n = 2000, list(ar = c(0, 0, 0), ma = c(0.2, -0.2, 0.2)),
                                                   sd = 1), arima.sim(n = i1, list(ar = -c(0.15, 0.15, 0.15), ma = -c(0.15, 0.15, 0.15)),
                                                                      sd = 1))
...
dis_matrix_q <- (1/(4 * l_lags * l_levels^2)) * dist(matrix_features_q)^2
plot_1 <- plot_2d_scaling(dis_matrix_q, cluster_labels = ground_truth)$plot
plot_1_auxiliary <- plot_1 + theme(axis.title = element_text(size = 10), axis.text = element_text(size = 10),
               legend.text = element_text(size = 10), plot.title = element_text(size = 12)) +
  ggtitle(TeX('Scenario 1 ($\\eta_1$)'))
shared_legend <- extract_legend(plot_1_auxiliary)

plot_1_final <- plot_1 + theme(axis.title = element_text(size = 10), axis.text = element_text(size = 10),
...
matrix_features_q <- list_to_matrix(features_q)
dis_matrix_q <- (1/(4 * l_lags * l_levels^2)) * dist(matrix_features_q)^2
plot_2 <- plot_2d_scaling(dis_matrix_q, cluster_labels = ground_truth)$plot

plot_2_final <- plot_2 + theme(axis.title = element_text(size = 10), axis.text = element_text(size = 10),
                               legend.position = 'none', plot.title = element_text(size = 12)) +
  ggtitle(TeX('Scenario 1 ($\\eta_2$)'))
