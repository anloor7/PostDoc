...
for (j in 1 : trials) {
  ...
  for (k1 in 1 : l_fp) {
    ...
    for (k2 in 1 : 200) {
      ...
      cl_acff <- fuzzy_c_medoids(X = cbind(c(feature_dataset_acff), c(rep(0, 20))), m = vector_fp[k1], C = K, dis = sed)$U
      cl_acffs <- fuzzy_c_medoids(X = cbind(c(feature_dataset_acffs), c(rep(0, 20))), m = vector_fp[k1], C = K, dis = sed)$U
...
mds2 <- isoMDS(dis_matrix_f_2)
mds2$stress
mds_2_a <- smacofSym(dis_matrix_f_2)
permtest(mds_2_a)
