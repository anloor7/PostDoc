...
for (i in 1 : 41) {
  ...
  list_2[[i]] <- list_1[[i]][list_1[[i]]$Year >= 1960, c(1, 2, 5)]
  list_2_p[[i]] <- list_1_p[[i]][list_1_p[[i]]$Year >= 1960, c(1, 2, 5)]
...
for (i in 1 : 41) {
  ...
  list_3[[i]][j,] <- list_2[[i]]$Total[indexes_j]
  list_3_p[[i]][j,] <- list_2_p[[i]]$Total[indexes_j]  
...
set.seed(1234)
clustering_km <- fuzzy_c_medoids(feature_dataset_fqa, C = 2, m = 1.5, dis = sed)
cl <- clustering_km$U
