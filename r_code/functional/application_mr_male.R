...
for (i in 1 : 41) {
  ...
  list_2[[i]] <- list_1[[i]][list_1[[i]]$Year >= 1960, c(1, 2, 4)]
  list_2_p[[i]] <- list_1_p[[i]][list_1_p[[i]]$Year >= 1960, c(1, 2, 4)]
...
for (i in 1 : 41) {
  ...
  for (j in 1 : length(years[[i]])) {
    ...
    list_3[[i]][j,] <- list_2[[i]]$Male[indexes_j]
    list_3_p[[i]][j,] <- list_2_p[[i]]$Male[indexes_j]  
