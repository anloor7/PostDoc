...
vector_j_auxiliary[k2] <- JACCARD.F(ground_truth, cl)
  ...
  for (k2 in 1 : 200) {
    ...
    cl_k_m <- fuzzy_c_medoids(X = feature_dataset_k_m, m = vector_fp[k1], C = K, dis = sed)$U
  ...
  for (k2 in 1 : 200) {
    ...
    cl_k_i <- fuzzy_c_medoids(X = feature_dataset_k_i, m = vector_fp[k1], C = K, dis = sed)$U
  ...
  for (k2 in 1 : 200) {
    ...
    cl_acff <- fuzzy_c_medoids(X = feature_dataset_acff, m = vector_fp[k1], C = K, dis = sed)$U
  ...
  for (k2 in 1 : 200) {
    ...
    cl_acffs <- fuzzy_c_medoids(X = feature_dataset_acffs, m = vector_fp[k1], C = K, dis = sed)$U
...
mds1 <- isoMDS(dis_matrix_f_1)
mds1$stress
mds_1_a <- smacofSym(dis_matrix_f_1)
permtest(mds_1_a)
