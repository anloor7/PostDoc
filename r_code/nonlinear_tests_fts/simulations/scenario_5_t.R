...
for (k in seq_along(c_values)) {
  ...
  for (i in 1:trials) {
    ...
    series <- simulate_linear_far1_heavy(series_length = series_length, 
                                   n_points = n_points, 
                                   phi_mat = phi_mat)
    f_data <- t(series)
...
# save(power_results, file = 'power_results_s5_2.RData')
...
plot_2 <- ggplot(df_long, aes(x = c, y = Power, color = Method, shape = Method)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  ...
  labs(title = "Scenario 5 (Student-t noise i.i.d.)",
       x = "",
       y = "",
       color = "Method",
       shape = "Method") +
