...
for (k in seq_along(sum_abs_values)) {
  ...
  for (i in 1:trials) {
    ...
    series <- simulate_threshold_far1(series_length = series_length, 
                                   n_points = n_points, 
                                   phi1_mat = phi_1_mat,
                                   phi2_mat = phi_2_mat,
...
# save(power_results, file = 'power_results_s6_1.RData')
...
plot_1 <- ggplot(df_long, aes(x = sum_abs_val, y = Power, color = Method, shape = Method)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
