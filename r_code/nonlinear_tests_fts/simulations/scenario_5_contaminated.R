...
for (k in seq_along(c_values)) {
  ...
  for (i in 1:trials) {
    ...
    series <- simulate_linear_far1_bm_contaminated(series_length = series_length, 
                                   n_points = n_points, 
                                   phi_mat = phi_mat, p_contam = 0.10, 
                                   h_vec = c(rep(0L, n_points-50), rep(1L, 50))[sample(n_points)])
    f_data <- t(series)
...
# save(power_results, file = 'power_results_s5_4.RData')
...
plot_4_prev <- ggplot(df_long, aes(x = c, y = Power, color = Method, shape = Method)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  ...
  labs(title = "Scenario 5 (Brownian motion noise with outliers)",
       x = "c",
...
shared_legend <- extract_legend(plot_4_prev)

plot_4 <- ggplot(df_long, aes(x = c, y = Power, color = Method, shape = Method)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_color_manual(values = default_cols) +
  scale_shape_manual(values = shapes) +
...
plot_total_prev <- grid.arrange(plot_1, plot_2, plot_3, plot_4)

plot <- grid.arrange(
  arrangeGrob(plot_total_prev), nrow = 2, ncol = 1, shared_legend,
  heights = c(40, 5))

savefig('power_1', width = 25, height = 20, type = 'png', toplines = 0.5)
plot <- grid.arrange(
  arrangeGrob(plot_total_prev), nrow = 2, ncol = 1, shared_legend,
  heights = c(40, 5))
dev.off()
